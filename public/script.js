    ...
    fetch(`${apiUrl}/get-attendance`)
        ...
        .then(messages => {
            attendanceData = messages.map(msg => ({
                name: msg.full_name,
                timestamp: new Date(msg.attendance_time).toLocaleString(),
                status: msg.status,
                course: msg.course_name || 'Tidak ada jadwal'
            }));
            messages.forEach(msg => courses.add(msg.course_name || 'Tidak ada jadwal'));
            fetchStudentList();
        })
